...
def journ(source)
  ...
  puts "Il y en a #{source.length}"
end 

def count(source)
  puts "Combien d'handle contiennent un numéro ?"
  puts "Il y a #{source.count{|source| source.match (/\d/)}}"
end 

def aude(source)
  puts "Combien d'handle contiennent les 4 lettres du prénom Aude à la suite (sans prendre en compte les majuscules) ?"
  puts "Il y en a : #{source.count{|source| source.match "[Aa]ude"}}"
end

def maj(source)
  puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
  puts "Il y a #{source.count{|source| source.match "@[A-Z]"}}"
end 

def first(source)
  puts "Combien commencent par une majuscule (première lettre juste après le @) ?"
  puts "Il y a : #{source.count{|source| source.match "[A-Z]"} } logins commençant par une majuscule"
end 

def underscore(source)
  puts "Combien y a-t-il de underscore dans tous les handles confondus ?"
